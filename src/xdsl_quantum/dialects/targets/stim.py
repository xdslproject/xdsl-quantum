...
@irdl_attr_definition
class QubitMappingAttr(ParametrizedAttribute):
    """
...
Stim contains a number of `Annotations` - instructions which do not affect
the operational semantics of the stim circuit - but may provide useful
information about the circuit being run or how decoding should be done.

These are essentially code-directives for compiler analyses on the circuit.

Here each is attached as an attribute instead - but as they may appear in the
code, are also given operations that can drive the change of a value and be
used to direct printing of stim circuits.
"""


class AnnotationOp(IRDLOperation, ABC):
    """
...
class SingleQubitGateOp(IRDLOperation, ABC):
    """
...
class TwoQubitGateOp(IRDLOperation, ABC):
    """
    ...
    def verify_(self) -> None:
        if len(self.targets) % 2:
            raise VerifyException(
                f"""Expected an even number of targets for {self.STIM_NAME},
                    got {len(self.targets)}"""
            )
...
class MeasurementOperation(IRDLOperation, ABC):
    """
...
class ResetOperation(IRDLOperation, ABC):
    """
...
class MeasureResetOperation(IRDLOperation, ABC):
    """
